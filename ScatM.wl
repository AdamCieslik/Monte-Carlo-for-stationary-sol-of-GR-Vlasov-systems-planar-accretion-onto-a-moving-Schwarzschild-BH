...
LengthScatM=Import[FileNameJoin[{notebookDir, "Data","dataLenghts.m"}]][[2]];
normalizScatM=LengthScatM*d\[Phi]; 
...
For[r0=2.00001, r0<=20.1, r0=r0+dr,
	JtScatResultsM=Table[0,{pixel,1,n\[Phi]}];
	JrScatResultsM=Table[0,{pixel,1,n\[Phi]}];
	JphiScatResultsM=Table[0,{pixel,1,n\[Phi]}];
	...
	For[j=1,j<=numberOfParts,j++,
		...
		fileNameScatM=FileNameJoin[{notebookDir, "Data", StringJoin["dataScatM_Part",ToString[j],".m"]}];
		(* Reading data from files *)
		scatM=Import[fileNameScatM,"Package"];
		...
		ScatMSubsets=Partition[scatM,UpTo[Ceiling[Length[scatM]/nSubsets]]];
		
		(* A function for processing a single subset of data *)
		processSubsetScatM[subset_]:=
		Module[{countsLocal=Table[0,{pixel,1,n\[Phi]}],countsrLocal=Table[0,{pixel,1,n\[Phi]}],countsphiLocal=Table[0,{pixel,1,n\[Phi]}]},
			...
			For[i=1,i<=Length[subset],i++,
				...
				If[r0>per,
					...
					weight=2 weight* voltotM2/normalizScatM;
					weightrIN=2  weightr*voltotM2/normalizScatM;
					weightphi=2 weightphi*voltotM2/normalizScatM;
		...
		allResultsScatM=ParallelMap[processSubsetScatM,ScatMSubsets];
		(*Combining results into individual lists with data*)
		finalCountsScatM=N[Total[allResultsScatM[[All,1]]]];
		finalCountsrScatM=N[Total[allResultsScatM[[All,2]]]];
		finalCountsphiScatM=N[Total[allResultsScatM[[All,3]]]];
		
		(*Accumulation of results for each loop run*)
		JtScatResultsM=JtScatResultsM+finalCountsScatM;
		JrScatResultsM=JrScatResultsM+finalCountsrScatM;
		JphiScatResultsM=JphiScatResultsM+finalCountsphiScatM;
		
		(* Memory clearing *) 
		Clear[scatM, ScatMSubsets];
		ParallelEvaluate[Clear[scatM, ScatMSubsets, dir, \[Lambda], \[Phi]0, \[Epsilon], per, psiIN, psiIN2, pixelin, weight, weightr, weightphi, weight, weightrIN, weightphi, allResultsScatM, finalCountsScatM, finalCountsrScatM, finalCountsphiScatM ]];
		CloseKernels[];
		ClearSystemCache[];
	...
	For[pixel=1,pixel<=n\[Phi],pixel++,
		...
		AppendTo[AllResultsJtScatM,{N[\[Phi]sr] ,JtScatResultsM[[pixel]]}];
		AppendTo[AllResultsJrScatM,{N[\[Phi]sr] ,JrScatResultsM[[pixel]]}];
		AppendTo[AllResultsJphiScatM,{N[\[Phi]sr] ,JphiScatResultsM[[pixel]]}];
		];
	
	(* Exporting data to external files *)
	Export[FileNameJoin[{notebookDir, "Results", StringJoin["JtScatM",ToString[licznik],".m"]}],{r0,AllResultsJtScatM},"Package"];
	Export[FileNameJoin[{notebookDir, "Results", StringJoin["JrScatM",ToString[licznik],".m"]}],{r0,AllResultsJrScatM},"Package"];
	Export[FileNameJoin[{notebookDir, "Results", StringJoin["JphiScatM",ToString[licznik],".m"]}],{r0,AllResultsJphiScatM},"Package"];
